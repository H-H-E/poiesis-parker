...
const memoryBuckets = [
  { id: "fun-facts", name: "Fun Facts", icon: <Gamepad2 className="size-5 text-purple-600" />, color: "bg-purple-100", activeColor: "bg-purple-200", textColor: "text-purple-700" },
  { id: "about-me", name: "About Me", icon: <Palette className="size-5 text-blue-600" />, color: "bg-blue-100", activeColor: "bg-blue-200", textColor: "text-blue-700" },
  { id: "learning", name: "Learning", icon: <Lightbulb className="size-5 text-green-600" />, color: "bg-green-100", activeColor: "bg-green-200", textColor: "text-green-700" },
  { id: "how-i-help", name: "How I Help", icon: <User className="size-5 text-yellow-600" />, color: "bg-yellow-100", activeColor: "bg-yellow-200", textColor: "text-yellow-700" },
  { id: "projects", name: "Projects", icon: <Rocket className="size-5 text-red-600" />, color: "bg-red-100", activeColor: "bg-red-200", textColor: "text-red-700" },
]
  ...
  return (
    <div className="space-y-6 rounded-lg bg-slate-50 p-1 dark:bg-gray-900/30">
      ...
      <div className="relative rounded-lg bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 p-5 text-white shadow-md">
        ...
        <h2 className="mb-1 text-xl font-bold">Welcome to Your Memory Journal!</h2>
              ...
              <div className="space-y-1">
                <Button
                  ...
                  className={cn(
                    ...
                    selectedBucket === "all" ? "bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100" : "text-slate-600 hover:bg-blue-100/50 dark:text-slate-300 dark:hover:bg-blue-900/50"
                  )}
                ...
                {memoryBuckets.map((bucket) => (
                  <Button
                    ...
                    className={cn(
                      "h-9 w-full justify-start rounded-md text-sm font-medium",
                      selectedBucket === bucket.name ? `${bucket.activeColor} ${bucket.textColor}` : "text-slate-600 hover:bg-blue-100/50 dark:text-slate-300 dark:hover:bg-blue-900/50"
                    )}
          ...
          <Card className="border-yellow-200 bg-yellow-50 shadow-sm dark:border-yellow-800 dark:bg-yellow-900/30">
            ...
            <CardContent className="space-y-3 p-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="remember-facts" className="flex items-center text-sm text-yellow-800 dark:text-yellow-200">
                   <span className="mr-2 text-yellow-600 dark:text-yellow-400">🔒</span> Remember Facts
                </Label>
                <Switch id="remember-facts" defaultChecked className="data-[state=checked]:bg-yellow-600 [&>span]:bg-yellow-400"/>
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="remember-preferences" className="flex items-center text-sm text-yellow-800 dark:text-yellow-200">
                   <span className="mr-2 text-yellow-600 dark:text-yellow-400">🔒</span> Remember Likes
                </Label>
                <Switch id="remember-preferences" defaultChecked className="data-[state=checked]:bg-yellow-600 [&>span]:bg-yellow-400"/>
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="remember-projects" className="flex items-center text-sm text-yellow-800 dark:text-yellow-200">
                   <span className="mr-2 text-yellow-600 dark:text-yellow-400">🔒</span> Remember Projects
                </Label>
                <Switch id="remember-projects" defaultChecked className="data-[state=checked]:bg-yellow-600 [&>span]:bg-yellow-400"/>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              <DialogTrigger asChild>
                <Button size="sm" className="h-9 rounded-md bg-gradient-to-r from-green-500 to-blue-500 text-white hover:from-green-600 hover:to-blue-600">
                  <Gift className="mr-2 size-4" /> Add Memory
                </Button>
              </DialogTrigger>
              <DialogContent className="bg-white sm:max-w-[425px] dark:bg-slate-900">
                <DialogHeader>
                  <DialogTitle className="text-center text-lg font-semibold text-blue-700 dark:text-blue-300">Add to Your Memory Journal</DialogTitle>
                   ...
                   <div className="space-y-2">
                     ...
                     <div className="flex flex-wrap gap-2">
                       {memoryBuckets.map((bucket) => (
                         ...
                         >
                           <span className="mr-1.5 shrink-0 scale-90">{bucket.icon}</span> {bucket.name}
                         </Button>
                       ))}
                     </div>
                   </div>
          ...
          <div className="space-y-4">
            {filteredMemories.length > 0 ? (
              filteredMemories.map((memory) => (
                <Card key={memory.id} className={cn("overflow-hidden border-2 shadow-sm", memory.color, "dark:border-opacity-30 dark:bg-opacity-10")}>
                  <CardContent className="flex items-start gap-4 p-4">
                    <div className={cn("flex size-10 shrink-0 items-center justify-center rounded-full", memory.color.replace("bg-", "bg-").replace("-100", "-200"), "dark:bg-opacity-30")}>
                       {memory.icon}
                    </div>
                    <div className="grow">
                       <div className="mb-1 flex items-center justify-between">
                          <Badge variant="outline" className={cn("rounded-full border px-2 py-0.5 text-xs font-medium", memory.color.replace("-100", "-300/70"), memory.textColor, "dark:border-opacity-50 dark:bg-opacity-20")}>
                             {memory.type}
                          </Badge>
                          <div className="flex items-center gap-1">
                             <Button variant="ghost" size="icon" className={cn("size-7 rounded-full", memory.textColor, "hover:bg-black/10 dark:hover:bg-white/10")} onClick={() => toggleStar(memory.id)}>
                                <Star className={cn("size-4", memory.isStarred ? "fill-current text-yellow-500" : "text-slate-400 dark:text-slate-500")}/>
                             </Button>
                             <Button variant="ghost" size="icon" className={cn("size-7 rounded-full text-red-500 hover:bg-red-100 dark:hover:bg-red-900/50")} onClick={() => deleteMemory(memory.id)}>
                                <Trash2 className="size-4" />
                             </Button>
                          </div>
                       </div>
                       <p className={cn("text-sm font-medium", memory.textColor, "dark:text-opacity-90")}>{memory.content}</p>
                       <p className={cn("mt-1 text-xs", memory.textColor, "opacity-80 dark:text-opacity-70")}>Added on {formatDate(memory.date)}</p>
                    </div>
                  </CardContent>
                </Card>
              ))
