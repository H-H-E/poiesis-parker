...
export default function AdminMemoryPage() {
  ...
  const fetchFactsForUser = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch memories: ${response.statusText} ${errorData?.error || ''}`.trim());
      }
      ...
      if (offset === 0 && availableFactTypes.length === 0 && data.facts?.length > 0) {
        const types = [...new Set(data.facts.map((fact: Fact) => fact.factType))].filter(Boolean) as string[];
  ...
  const handleSaveFact = async (updatedFact: Fact) => {
    try {
      ...
      const endpoint = isCreating ? '/api/admin/createstudentfact' : '/api/admin/updatestudentfact';
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(`Failed to ${isCreating ? 'create' : 'update'} fact: ${response.statusText} ${errorData?.error || ''}`.trim());
      }
  ...
  const handleDeleteFact = async (factId: string) => {
    if (confirm('Are you sure you want to delete this fact? This action cannot be undone.')) {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(`Failed to delete fact: ${response.statusText} ${errorData?.error || ''}`.trim());
        }
  ...
  return (
    <main className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Admin Memory Management</h1>
      ...
      {selectedUserId && (
        <div className="mt-6">
          <h2 className="mb-4 text-xl font-semibold">Memory for User ID: {selectedUserId}</h2>
          ...
          {!isLoadingFacts && facts.length > 0 && !hasMore && (
             <p className="mt-8 text-center text-sm text-gray-500">End of results.</p>
          )}
        </div>
      )}

      {!selectedUserId && (
        <p className="mt-6 text-gray-600">Please select a student to view their memory.</p>
      )}
