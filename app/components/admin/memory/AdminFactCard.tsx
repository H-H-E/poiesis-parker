...
import { formatDate, getConfidenceClasses, getConfidenceLabel, truncateText } from '@/app/lib/memory/admin-utils';
...
export const AdminFactCard: React.FC<AdminFactCardProps> = ({ fact, onEdit, onDelete }) => {
  const timeAgo = formatDistanceToNow(new Date(fact.updatedAt), { addSuffix: true });
  ...
  const getTypeStyle = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case 'struggle': return 'bg-orange-100 text-orange-800';
      case 'goal': return 'bg-indigo-100 text-indigo-800';
      case 'topic_interest': return 'bg-pink-100 text-pink-800';
      case 'learning_style': return 'bg-teal-100 text-teal-800';
  ...
  return (
    <div className="mb-4 rounded-lg border p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-full px-2 py-0.5 ${getConfidenceClasses(fact.confidence)}`}>
            {getConfidenceLabel(fact.confidence)}
          </span>
        </div>
        <span className="text-gray-500" title={exactDate}>Updated {timeAgo}</span>
      </div>
      ...
      <div className="mb-3 space-y-1 text-xs text-gray-600">
        ...
        {fact.originContext && (
          <p><span className="italic">Source: {truncateText(fact.originContext, 150)}</span></p>
        )}
        <p>ID: {fact.id}</p> 
      </div>
