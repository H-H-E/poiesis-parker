...
const mapDbMessageToLangChainMessage = (
  msg: Tables<"messages">
): BaseMessage => {
  ...
  switch (lcRole) {
    ...
    case "tool": {
      ...
      console.warn(
        "Mapping DB 'tool' role to generic ChatMessage due to missing tool_call_id in schema."
      )
...
export class SupabaseChatMessageHistory extends BaseChatMessageHistory {
  ...
  async addMessage(message: BaseMessage): Promise<void> {
    ...
    const { data: latestMessages, error: fetchError } =
  ...
  async pruneHistory(keepSequenceNumberAfter: number): Promise<void> {
    console.log(
      `Pruning history for chat ${this.chatId}, keeping messages after sequence number ${keepSequenceNumberAfter}`
    )
