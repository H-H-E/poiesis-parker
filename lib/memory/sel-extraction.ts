        ...
        functions: [
            {
                ...
                description: "Extracts communication style preferences from student conversations.",
...
export function createTopicEngagementExtractionChain({
    ...
    const extractionLlm = llm.bind({
        functions: [
            {
                name: extractionFunctionName,
                description: "Extracts topic engagement patterns from student conversations.",
...
export function createSocialEmotionalExtractionChain({
    ...
    const extractionLlm = llm.bind({
        functions: [
            {
                name: extractionFunctionName,
                description: "Extracts social-emotional indicators from student conversations.",
...
export function createComprehensiveExtractionChain({
    ...
    const extractionLlm = llm.bind({
        functions: [
            {
                name: extractionFunctionName,
                description: "Extracts comprehensive insights about a student from conversations.",
...
export async function extractComprehensiveInsightsFromMessages({
    ...
    try {
        return await chain.invoke({ conversation_text: conversationText }) as ComprehensiveInsights;
    } catch (error) {
 ...
 */
export function formatCommunicationStylePreferencesForPrompt(styles: CommunicationStyle[] = []): string {
 ...
 */
export function formatTopicEngagementForPrompt(topics: TopicEngagement[] = []): string {
    ...
    const formattedTopics = topics.map(topic => {
        ...
        switch (topic.engagement_level) {
            case "high": engagementDesc = "Shows high interest in"; break;
            case "moderate": engagementDesc = "Is moderately interested in"; break;
            case "low": engagementDesc = "Shows little interest in"; break;
            case "negative": engagementDesc = "Reacts negatively to"; break;
        }

        const contextStr = topic.context ? ` when ${topic.context}` : '';
        const exceptionStr = topic.exception ? ` Exception: ${topic.exception}` : '';
 ...
 */
export function formatSocialEmotionalForPrompt(indicators: SocialEmotional[] = []): string {
 ...
 */
export function formatAllSELInsightsForPrompt(insights: Partial<ComprehensiveInsights> = {}): string {
    ...
    if (insights.communication_styles && insights.communication_styles.length > 0) {
        sections.push(formatCommunicationStylePreferencesForPrompt(insights.communication_styles));
    }
