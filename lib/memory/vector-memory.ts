...
export async function ingestConversationHistory({
  ...
  if (docs.length === 0) {
    console.log(`Splitting produced no documents for chat ${chatId}, skipping ingestion.`);
    return;
  }

  // Use explicit API key if provided (server-side context)
  const embeddings = new OpenAIEmbeddings({ openAIApiKey: embeddingApiKey ?? process.env.OPENAI_API_KEY });
  ...
  try {
    ...
    console.log(`Ingested ${docs.length} chunks for chat ${chatId}. IDs: ${ids.join(', ')}`);
    return ids;
  } catch (error) {
