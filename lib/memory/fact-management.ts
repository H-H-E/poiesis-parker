...
export async function updateStudentFact({
  ...
  factId: string
  updates: Partial<
    Omit<StudentFact, "id" | "user_id" | "created_at" | "updated_at">
  > // Allow updating relevant fields
  client?: SupabaseClient
...
export async function detectAndHandleFactConflicts({
  ...
  console.log(
    `Found ${existingFacts.length} potential conflicts. Using strategy: ${strategy}`
  )
  ...
  switch (strategy) {
    case "prefer_new": {
      ...
      if (addError) {
        throw new Error(
          `Failed to add new fact after deactivating conflicts: ${addError.message}`
        )
      }
    ...
    case "prefer_high_confidence": {
      ...
      if ((newFact.confidence || 0) > (highestConfidenceFact.confidence || 0)) {
        ...
        const { data: addedHighConfFact, error: addHighConfError } =
          await client
            .from("student_facts")
            .insert({ ...newFact, user_id: userId })
            .select()
        ...
        if (addHighConfError) {
          throw new Error(
            `Failed to add higher confidence fact: ${addHighConfError.message}`
          )
        }
...
export async function getContextuallyRelevantFacts({
  ...
  try {
    ...
    let query = client.from("student_facts").select("*").eq("user_id", userId)
    ...
    const getRelevanceScore = (
      ...
      const contextTerms = lowerContext
        .split(/\s+/)
...
export async function identifyKnowledgeGaps({
  ...
  try {
    ...
    const factTypeCounts: Record<FactType, number> = {} as Record<
      FactType,
      number
    >
    ...
    for (const fact of facts || []) {
      ...
      // Count by fact type
      factTypeCounts[typedFact.fact_type] =
        (factTypeCounts[typedFact.fact_type] || 0) + 1

      // Count by subject
      if (typedFact.subject) {
        subjectCounts[typedFact.subject] =
          (subjectCounts[typedFact.subject] || 0) + 1
      }
    }
    ...
    if (missingFactTypes.includes("preference")) {
      recommendedQuestions.push(
        "What are your preferences for learning content?"
      )
    }
    if (missingFactTypes.includes("struggle")) {
      recommendedQuestions.push(
        "What areas or topics do you find most challenging?"
      )
    }
    if (missingFactTypes.includes("goal")) {
      recommendedQuestions.push(
        "What are your learning goals or what do you hope to achieve?"
      )
    }
    if (missingFactTypes.includes("learning_style")) {
      recommendedQuestions.push(
        "How do you prefer to learn? (e.g., visual, hands-on, reading)"
      )
    }
    if (missingFactTypes.includes("topic_interest")) {
      recommendedQuestions.push(
        "What topics or subjects are you most interested in?"
      )
    }
...
export async function updateFactTags({
  ...
  try {
    ...
    if (factError || !fact) {
      ...
      throw new Error(
        `Fact not found: ${factError?.message || "No fact with this ID"}`
      )
    }
...
export async function generateUserKnowledgeProfile({
  ...
  try {
    ...
    // Count facts by type
    const factTypeDistribution: Record<FactType, number> = {} as Record<
      FactType,
      number
    >
    const factsByType: Record<FactType, StudentFact[]> = {} as Record<
      FactType,
      StudentFact[]
    >
    ...
    for (const fact of typedFacts) {
      // Count by type
      factTypeDistribution[fact.fact_type] =
        (factTypeDistribution[fact.fact_type] || 0) + 1
    ...
    // Intro
    summaryParts.push(
      `This user has ${typedFacts.length} stored facts across ${Object.keys(factTypeDistribution).length} categories.`
    )
    ...
    if (factsByType.topic_interest && factsByType.topic_interest.length > 0) {
      const interestSummary = factsByType.topic_interest
        .slice(0, maxFactsPerType)
        .map(fact =>
          fact.subject ? `${fact.subject} (${fact.details})` : fact.details
        )
        .join("; ")
    ...
    if (recentSubjects.length > 0) {
      summaryParts.push(
        `Recent subjects: ${recentSubjects.slice(0, 5).join(", ")}.`
      )
    }
...
export async function analyzeStudentFactPatterns({
  ...
  try {
    ...
    const preferences = typedFacts.filter(
      fact => fact.fact_type === "preference"
    )
    const struggles = typedFacts.filter(fact => fact.fact_type === "struggle")
    const goals = typedFacts.filter(fact => fact.fact_type === "goal")
    const interests = typedFacts.filter(
      fact => fact.fact_type === "topic_interest"
    )
    const learningStyles = typedFacts.filter(
      fact => fact.fact_type === "learning_style"
    )
    ...
    if (factTypeCounts.goal && factTypeCounts.goal > 2) {
      learningPatterns.push(
        "Goal-oriented learner who benefits from clear objectives"
      )
    }

    // Pattern: Struggle-aware learner
    if (
      factTypeCounts.struggle &&
      factTypeCounts.struggle > factTypeCounts.goal
    ) {
      learningPatterns.push(
        "Focuses more on challenges than goals - may benefit from strengths-based approach"
      )
    }

    // Pattern: Interest-driven learner
    if (
      factTypeCounts.topic_interest &&
      factTypeCounts.topic_interest > (factTypeCounts.goal || 0)
    ) {
      learningPatterns.push(
        "Interest-driven learner who engages best with topics of personal relevance"
      )
    }
    ...
    if (goals.length > 0) {
      engagementSuggestions.push(
        "Connect learning activities to their stated goals for increased motivation"
      )
    }
    ...
    if (interests.length > 0) {
      engagementSuggestions.push(
        "Use topics of interest as examples or contexts for teaching new concepts"
      )
    }
    ...
    if (learningStyles.length > 0) {
      engagementSuggestions.push(
        "Accommodate their preferred learning style when presenting new information"
      )
    }
    ...
    if (challenges.length > 0) {
      engagementSuggestions.push(
        "Provide extra support for identified challenge areas while building on strengths"
      )
    }
